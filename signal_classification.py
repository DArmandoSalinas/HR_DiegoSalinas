...
def process_gsr_data(filename, ground_truth_file):
    ...
    feature_functions = {
        ...
        'skewness': skew,
        'kurtosis': kurtosis,
        #'ZeroCrossings' : librosa.zero_crossings,
        'MeanEnergy' : np.average,
        'thirdmoment' : lambda x : stats.moment(x, moment = 3) 
    ...
    for group_key, group_df in grouped:
        ...
        #for col in ['Phasic Signal', 'Tonic Signal', 'GSR RAW', 'GSR Resistance CAL', 'GSR Conductance CAL']:
        for col in ['Phasic Signal', 'Tonic Signal']:   
            for feature_name, feature_func in feature_functions.items():
                feature_value = feature_func(group_df[col])
...
def plot_learning_curve(model, X_train, y_train, X_test, y_test, save_filename=None):
    ...
    save_filename = "C:/Users/ZeelP/Documents/901_EEG-master/figure_name.png"
...
def train_evaluate_models(features, target):
    ...
    bayesian_rscore = r2_score(y_test, y_pred_bayesian)
    print("bayesian_rscore", bayesian_rscore)
    ...
    DT_rscore = r2_score(y_test, y_pred_tree)
    print("knn_rscore", DT_rscore)
    ...
    keras_rscore = r2_score(y_test, y_pred_keras)
    print("keras_rscore", keras_rscore)
    ...
    print("keras rscore", keras_rscore)
    print("rf rscore", rf_rscore)
    print("gb rscore", gb_rscore)
    print("bayesian rscore", bayesian_rscore)
    print("knn rscore", knn_rscore)
    print("DT rscore", DT_rscore)
